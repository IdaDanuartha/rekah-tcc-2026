...
export async function submitDeliveryProof(
  ...
  // Bungkus semua: throw apa pun (env kurang, tabel/bucket belum ada, jaringan)
  // dikembalikan sbg {ok:false} — bukan crash Server Action (React #441).
  try {
    ...
    console.error("[submitDeliveryProof] gagal:", err);
    return { ok: false, error: err instanceof Error ? err.message : "Gagal menyimpan bukti." };
  }
}
